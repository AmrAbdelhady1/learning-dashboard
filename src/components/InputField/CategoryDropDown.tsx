...
interface Props {
  ...
  setSelectedItem: (item: Category) => void;
  data: Category[];
}

const CategoryDropDown = ({ selectedItem, setSelectedItem, data }: Props) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <p className="text-sm capitalize text-gray600">{t("Category")}</p>
      ...
      >
        <p>
          {selectedItem ? (
            ...
            <span className="text-gray-500">{t("Please select the category")}</span>
          )}
        </p>
